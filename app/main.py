class Person:
    ...
    def __repr__(self: str) -> any:
        spouse_name = getattr(self, "wife", None) or \
            getattr(self, "husband", None)
        spouse_name = spouse_name.name if spouse_name else None
        spouse_type = "wife" if hasattr(self, "wife") else "husband"
        return f"Person(name={self.name}, age={self.age}" + (
            f", {spouse_type}={spouse_name}" if spouse_name else "") + ")"


def create_person_list(people_list: str) -> str:
    person_instances = []
    ...
    for person_dict, person_instance in zip(people_list,
                                            person_instances):
